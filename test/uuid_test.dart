...
void main() {
  ...
  group('[Version 4 Tests]', () {
    test('Check if V4 is consistent using a static seed', () {
      var u0 = uuid.v4(options: {'rng': MathRNG(seed: 1)});
    ...
    test('Consistency check with buffer', () {
      var buffer = Uint8List(16);
      uuid.v4buffer(buffer, options: {'rng': MathRNG(seed: 1)});
    ...
    test('Using Objects', () {
      var regular = uuid.v4(options: {'rng': MathRNG(seed: 1)});
      var obj = uuid.v4obj(options: {'rng': MathRNG(seed: 1)});
  ...
  group('[Version 5 Tests]', () {
    ...
    test('String vs binary produces same UUIDs for equivalent UTF-8', () {
      ...
      var binaryName = Uint8List.fromList([
        0x74,
        0x65,
        0x73,
        0x74,
  ...
  group('[Version 6 Tests]', () {
    test('IDs created at same mSec are different', () {
      expect(
        ...
        isNot(
          equals(uuid.v6(config: V6Options(null, testTime, null, null, null))),
        ),
      );
    });
    ...
    test('Clock regression by msec increments the clockseq - mSec', () {
      ...
      var uidtb = uuid.v6(
        config: V6Options(null, testTime - 1, null, null, null),
      );
  ...
  group('[Parse/Unparse Tests]', () {
    ...
    test('Parsing a dirty string with a UUID in it', () {
      var id = '00112233445566778899aabbccddeeff';
      expect(
        () => Uuid.unparse(Uuid.parse('(this is the uuid -> $id$id')),
        throwsA(isA<FormatException>()),
      );
    });
      ...
      for (final testCase in {
        ...
        test(testCase.key, () {
          final v = Uuid.parse(
            Namespace.oid.value,
            buffer: buffer,
            offset: testCase.value,
          );

          expect(
            Uuid.unparse(v, offset: testCase.value),
  ...
  group('[UuidValue]', () {
    ...
    test('Pass invalid Uuid to constructor', () {
      ...
      expect(
        () => UuidValue.withValidation(invalidUUID),
        throwsA(isA<FormatException>()),
      );
    ...
    test('Pass valid Guid to constructor with validation mode nonStrict', () {
      ...
      expect(
        Uuid.isValidUUID(
          fromString: validGUID,
          validationMode: ValidationMode.nonStrict,
        ),
        true,
      );

      final uuidval = UuidValue.withValidation(
        validGUID,
    ...
    test('Pass valid Ulid to constructor with format validation', () {
      ...
      expect(Uuid.isValidUUID(fromString: validUlid), false);
      expect(Uuid.isValidUUIDFormat(fromString: validUlid), true);

      final uuidval = UuidValue.withFormatValidation(validUlid);
  ...
  group('[Validation Test]', () {
    ...
    test('No Dashes UUID', () {
      ...
      expect(
        Uuid.isValidUUID(fromString: validNoDashesUUID, noDashes: true),
        true,
      );
    ...
    test('No Dashes UUIDValue Formatted string validate', () {
      ...
      var uuidval = UuidValue.fromString(validNoDashesUUID);
      expect(
        uuidval.toFormattedString(validate: true),
        "87cd4eb3-cb88-449b-a1da-e468fd829310",
      );
    });
    ...
    group('[UUID7]', () {
      for (final testCase in {
        ...
        test(testCase.key, () {
          ...
          final uuid = Uuid().v7(
            config: V7Options(testCase.value[0] as int, rand),
          );
          expect(uuid.toUpperCase(), equals(testCase.value[1]));
        });
      }
    ...
    group('[UUID8]', () {
      for (final testCase in {
        ...
        test(testCase.key, () {
          final rand = MathRNG(seed: 1).generate();
          final uuid = Uuid().v8(
            config: V8Options(testCase.value[0] as DateTime, rand),
          );
          expect(uuid.toUpperCase(), equals(testCase.value[1]));
        });
      }
      ...
      for (final testCase in {
        ...
        test(testCase.key, () {
          final uuid = Uuid().v8g(
            config: V8GenericOptions(testCase.value[0] as List<int>),
          );
          expect(uuid.toUpperCase(), equals(testCase.value[1]));
        });
      }
